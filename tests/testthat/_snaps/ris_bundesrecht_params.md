...
# ris_normalize_bundesrecht_sort_column errors on invalid value

    Code
      risAT:::ris_normalize_bundesrecht_sort_column("Datum")
    Condition
      Error in `risAT:::ris_normalize_bundesrecht_sort_column()`:
      ! `sort_by` is invalid. Use one of: 'ArtikelParagraphAnlage', 'Kurzinformation', 'Inkrafttretensdatum', 'Ausserkrafttretensdatum'.

# ris_req_bundesrecht rejects version_date combined with a date range

    Code
      ris_req_bundesrecht(version_date = "2020-01-01", effective_from = "2019-01-01")
    Condition
      Error in `ris_req_bundesrecht()`:
      ! `version_date` (point-in-time version) cannot be combined with `effective_*` / `expiry_*` date ranges. Use one or the other.

# ris_req_bundesrecht rejects malformed date strings
...
# ris_req_bundesrecht rejects non-string params
...
# ris_perform_bundesrecht rejects requests without ris_meta

    Code
      ris_perform_bundesrecht(plain_req)
    Condition
      Error in `ris_perform_bundesrecht()`:
